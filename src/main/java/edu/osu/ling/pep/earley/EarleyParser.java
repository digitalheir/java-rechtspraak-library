...
public class EarleyParser {
    ...
    public final ParserListener listener;
    public final Map<ParserOption, Boolean> options;
    ...
    public Parse parse(List<String> ts, Category seed) throws PepException {
        return parse(ts.stream().map(Token::new).collect(Collectors.toList()), seed);
    }
     ...
     *                      </ul>
     */
    public Parse parse(Iterable<Token> tokens, Category seed)
            throws PepException {
            ...
            for (Edge edge : edges.toArray(new Edge[edges.size()])) {
                ...
                if (!edge.isPassive()) {
                    ...
                    if (dr.activeCategory.terminal
                            && (dr.activeCategory.name.equals(token.toString())
                            || (ignoreCase
                            && dr.activeCategory.name.equalsIgnoreCase(
                            token.toString())))) {
                        Edge newEdge = Edge.scan(edge, token.toString());
            ...
            while (passiveEdgesToCompleteOn != null && passiveEdgesToCompleteOn.size() > 0) {
                ...
                for (Edge edge : passiveEdgesToCompleteOn) {
                    ...
                    for (Edge edgeToAdvance : chart.getEdgesWithActiveCategory(origin, edge.dottedRule.left)) {
                        ...
                        newEdges = Collections3.add(newEdges, newEdge);
                    }
                }
