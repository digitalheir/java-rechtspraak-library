...
public class CrfFeatureValueExpectationByModel<K, G> {
    ...
    private void addValueForSentence(List<? extends TaggedToken<K, G>> sentence) {
        ...
        // Calculate the CRF formula: e^{\Sum_{i=0}^{F-1}\theta_i*f_i(j,g,g')} where F is the numbering of features, j is a token index, g is a tag for that token, and g' is a tag for the previous token.
                ...
                for (G previousTag : possiblePreviousTags) {
                    ...
                    for (int featureIndex : activeFeatures) {
                        ...
                        if (featureValue != 0.0) {
                            ...
                            double addToExpectation = CrfUtilities.safeMultiply(featureValue, probabilityUnderModel);

                            if (!Double.isFinite(addToExpectation))
                                throw new Error("Illegal value: " + featureValue + " * " + probabilityUnderModel);

                            double old = featureValueExpectation[featureIndex];
                            featureValueExpectation[featureIndex] = safeAdd(old, addToExpectation);
                            if (!Double.isFinite(featureValueExpectation[featureIndex])) throw new Error(
                                    "Illegal value: safeAdd("
                                            + old
                                            + ", "
                                            + addToExpectation
                                            + ");");
