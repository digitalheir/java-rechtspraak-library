 ...
 * where \Psi(j,g,g') is the formula for token numbering j where its tag is g and the tag of token (j-1) is g', i.e.,
 * e^{\Sum{i=0}^{numbering-of-features}(\theta_i*f_i(j,g,g'))}
 * where \theta_i is parameter numbering i (these are the parameters learned in the training) and f_i is feature numbering i (the
...
public class CrfForwardBackward<K, G> {
    ...
    private void calculateAlphaForward() {
        ...
        for (int index = 0; index < sentence.length; ++index) {
            Map<G, Double> alpha_forwardThisToken = new LinkedHashMap<>();
            for (G tag : model.getCrfTags().getTags()) {
                ...
                for (G previousTag : previousTags) {
                    ...
                    if (index > 0) {
                        ...
                        valueForPreviousTag = CrfUtilities.safeMultiply(valueForPreviousTag, previousAlphaValue);
                    }
                    sumOverPreviousTags = CrfUtilities.safeAdd(sumOverPreviousTags, valueForPreviousTag);

                    if (!Double.isFinite(sumOverPreviousTags))
                        throw new Error("Invalid value: += " + valueForPreviousTag);
                }
        ...
        for (G tag : alphaLast.keySet()) {
            finalAlpha = CrfUtilities.safeAdd(finalAlpha, alphaLast.get(tag));


            if (!Double.isFinite(finalAlpha))
                throw new Error("Illegal alpha: " + finalAlpha + " - " + alphaLast.get(tag));
        }
    }


    private void calculateBetaBackward() {
        beta_backward = new ArbitraryRangeArray<>(sentence.length + 1, -1); // i.e. [-1,0,1,2,...,sentence.length-1]
        beta_backward.set(sentence.length - 1, new LinkedHashMap<>());
        for (G tag : model.getCrfTags().getTags()) {
            beta_backward.get(sentence.length - 1).put(tag, 1.0);
        }

        for (int index = sentence.length - 2; index >= (-1); --index) {
            LinkedHashMap<G, Double> betaCurrentToken = new LinkedHashMap<>();

            Set<G> currentTokenPossibleTags;
            if (index < 0) {
            ...
            for (G tag : currentTokenPossibleTags) {
                ...
                for (G nextTag : model.getCrfTags().getCanFollow().get(tag)) {
                    ...
                    sum = CrfUtilities.safeAdd(sum, valueForNextTag);
                    if (!Double.isFinite(sum))
                        throw new Error("Invalid value: " + sum);
                }
