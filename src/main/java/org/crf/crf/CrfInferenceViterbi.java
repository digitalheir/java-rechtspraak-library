 ...
 * where \psi_j(g,g') is the formula for token numbering j in the sequence, where its tag is s, and the tag of token numbering j-1 is g'.<BR>
 * This formula, in CRF, is e^{\sum_{i=0}^{k-1}{\theta_i*f_i(sequecne,j,g,g')}}/Z(sequence)<BR>
 * where k is the numbering of features, theta_i is the parameter numbering i, and f_i is feature numbering i,
...
public class CrfInferenceViterbi<K, G> {
    ...
    private void calculateViterbi() {
        ...
        for (int index = 0; index < sentence.length; ++index) {
            Map<G, Double> delta_viterbiForwardCurrentToken = new LinkedHashMap<>();
            argmaxTags[index] = new LinkedHashMap<>();
            for (G tag : model.getCrfTags().getTags()) {
                Set<G> tagsOfPrevious; // The set of tags that can be assigned to token index-1.
                if (0 == index) {
                ...
                argmaxTags[index].put(tag, tagOfPreviousWithMaxValue); // i.e. If the tag for token numbering "index" is "tag", then the tag for token "index-1" is "tagOfPreviousWithMaxValue".
