package org.leibnizcenter.rechtspraak.enricher.crf.features.textpatterns;
...
public class TitlePatterns {
    public static void setFeatureValues(Token t, List<TokenTreeLeaf> tokens, int ix) {
        // General title features (start with "feit", start with "standpunt")
        Features.setFeatures(
                t, tokens, ix,
                (NamedElementFeatureFunction[]) General.values());

        // Regex patterns for titles with high confidence
        ...
        if (matchesHighConf.size() > 0)  t.setFeatureValue("matchesHighConfTitle", 1.0);

        // Regex patterns for titles with low confidence
