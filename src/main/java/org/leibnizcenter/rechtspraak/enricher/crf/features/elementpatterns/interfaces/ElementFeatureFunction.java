package org.leibnizcenter.rechtspraak.enricher.crf.features.elementpatterns.interfaces;
...
public interface ElementFeatureFunction {
    static ElementFeatureFunction matches(Pattern patt) {
        return (elements, i) -> patt.matcher(elements.get(i).getTextContent()).matches();
    }

    static ElementFeatureFunction find(Pattern patt) {
        return (elements, i) -> patt.matcher(elements.get(i).getTextContent()).find();
    }
